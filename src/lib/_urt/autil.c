...
typedef struct ACS_memrec_ *ACS_memrec;

struct ACS_memrec_
{
    ACS_memrec next, prev;
    ULONG      size;
#ifdef MEMDEBUG
    ULONG      marker1;
#endif
...
static ACS_memrec g_mem_first, g_mem_last = NULL;
...
APTR ALLOCATE_(ULONG size, ULONG flags)
{
    ...
    ACS_memrec mr = (ACS_memrec) AllocMem (size + MR_OVERHEAD, flags);
    if (!mr)
    {
...
{
    BYTE *p = (BYTE*)ptr;
    ACS_memrec mr = (ACS_memrec) (p - MR_MEM_OFFSET);
    ULONG size = mr->size;

    DPRINTF ("DEALLOCATE: p=0x%08lx, mr=0x%08lx, size=%ld\n", p, mr, size);

#ifdef MEMDEBUG
    _ACS_ASSERT (mr->marker1 == MARKER1, (STRPTR) "DEALLOCATE: MARKER1 damaged");
    //DPRINTF ("DEALLOCATE: mr->marker1=0x%08lx\n", mr->marker1);
    ULONG *m2 = (ULONG*)(p + size);
    //DPRINTF ("DEALLOCATE: m2=0x%08lx\n", m2);
    _ACS_ASSERT (*m2 == MARKER2, (STRPTR) "DEALLOCATE: MARKER2 damaged");
    //DPRINTF ("DEALLOCATE: *m2=0x%08lx\n", *m2);
#endif
...
void ERROR (SHORT errcode)
{
    ...
    {
        __debug_puts((UBYTE*)"*** unhandled runtime error code: "); __debug_put_int16(errcode);
        __debug_puts((UBYTE*)"\n");
    }
...
void _autil_shutdown(void)
{
    ...
    ACS_memrec mr = g_mem_first;
    while (mr)
    {
        ACS_memrec mr_next = mr->next;
        ULONG size = mr->size;
