...
static FE_SLE slePush(FE_sleKind kind, S_pos pos, Tr_level lv, E_env env, Temp_label exitlbl, Temp_label contlbl, E_enventry ret_ve)
{
...
static FE_udtEntry FE_UDTEntryMethod(S_pos pos, Ty_proc proc)
{
    FE_udtEntry p = checked_malloc(sizeof(*p));
...
static E_enventry autovar(S_symbol v, S_pos pos)
{
    ...
    E_enventry x = E_resolveVar (g_sleStack->env, v);
...
static bool transConstDecl(S_pos pos, Ty_ty t, S_symbol name, Tr_exp cExp, bool isPrivate)
{
    ...
    S_enter(g_sleStack->env->venv, name, x);
    if (!isPrivate)
        S_enter(g_mod->env->venv, name, x);
...
static Tr_expList transActualArgs(S_tkn *tkn, Ty_proc proc);
...
static bool varDesignator(S_tkn *tkn, Tr_exp *var, bool doDeRef)
{
    if (!(*tkn))
        return FALSE;
    ...
    E_enventry x = autovar((*tkn)->u.sym, pos);
    if (x->kind != E_varEntry)
    {
        if (x->kind == E_constEntry)
        {
            *var = Tr_constExp(x->u.cExp);
            *tkn = (*tkn)->next;
            return TRUE;
        }
        return EM_error(pos, "this is not a variable: %s", S_name((*tkn)->u.sym));
    }

    *var = Tr_Var(x->u.var.access);
    *tkn = (*tkn)->next;
    ...
    if ((ty->kind == Ty_varPtr) && (ty->u.pointer->kind == Ty_procPtr) && ((*tkn)->kind==S_LPAREN))
    {
        ...
        *tkn = (*tkn)->next;    // skip "("
        ...
        Tr_expList assignedArgs = transActualArgs(tkn, proc);

        if ((*tkn)->kind != S_RPAREN)
            return EM_error((*tkn)->pos, ") expected.");
        *tkn = (*tkn)->next;

        *var = Tr_callPtrExp(*var, assignedArgs, proc);
        return TRUE;
    }
...
static bool atom(S_tkn *tkn, Tr_exp *exp)
{
    ...
    else
    {
        if ((*tkn)->kind == S_AT)                               // @v
        {
            ...
            *tkn = (*tkn)->next;
            ...
            if (!varDesignator(tkn, &e, /*doDeRef=*/FALSE))
                return FALSE;
    ...
    switch ((*tkn)->kind)
    {
        ...
        case S_IDENT:
        {
            ...
            // is this a declared variable?

            E_enventry x = E_resolveVar (g_sleStack->env, sym);
            if (x && ( ((*tkn)->next->kind != S_LPAREN) || (x->u.var.ty->kind == Ty_array) ) )
            {
                if (!varDesignator(tkn, exp, /*doDeRef=*/TRUE))
            ...
            E_enventryList lx = E_resolveFunc (g_sleStack->env, sym);
            if (lx)
            {
                E_enventry x = NULL;
...
static bool typeDesc (S_tkn *tkn, bool allowForwardPtr, Ty_ty *ty)
{
    ...
    if (isSym (*tkn, S_SUB) || isSym (*tkn, S_FUNCTION))
    {
        ...
        *tkn = (*tkn)->next;
        ...
        Ty_proc proc = Ty_Proc(/*name=*/ NULL, /*extra_syms=*/NULL, /*label=*/NULL, /*isPrivate=*/ TRUE, paramList->first, /*isStatic=*/ FALSE, returnTy, /*forward=*/ FALSE, /*offset=*/ 0, /*libBase=*/ NULL);
...
static bool transVarDecl(S_pos pos, S_symbol sVar, Ty_ty t, Tr_exp init, bool shared, bool statc, bool external, bool isPrivate, FE_dim dims)
{
    ...
    S_symbol name = sVar;
    S_scope  venv = g_sleStack->env->venv;
    if (shared)
    {
        assert(!statc);
        x = S_look(venv, name);
        ...
        x = S_look(g_mod->env->venv, name);
        if (x)
            return EM_error(pos, "Variable %s already declared in global scope.", S_name(name));
    ...
    else
    {
        ...
        {
            x = E_VarEntry(name, Tr_allocVar(g_sleStack->lv, S_name(name), t), t, FALSE);
            S_enter(venv, name, x);
        }
    }
...
static bool stmtAssignment(S_tkn *tkn)
{
    ...
    if (!varDesignator(tkn, &var, /*doDeRef=*/TRUE))
        return FALSE;
    ...
    *tkn = (*tkn)->next;
    ...
    {
        if (ty->kind != Ty_pointer)
            return EM_error(pos, "Pointer type expected here.");
        var = Tr_Deref(var);
        ty = Tr_ty(var);
    }

    if (!convert_ty(exp, ty, &convexp, /*explicit=*/FALSE))
        return EM_error(pos, "type mismatch (assign).");

    emit(Tr_assignExp(var, convexp, Tr_ty(var)));
...
static bool stmtForBegin(S_tkn *tkn, E_enventry e, Tr_exp *exp)
{
    ...
    *tkn = (*tkn)->next;           // consume "FOR"
    ...
    FE_SLE     sle     = slePush(FE_sleFor, pos, g_sleStack->lv, lenv, forexit, forcont, g_sleStack->ret_ve);
    ...
    *tkn = (*tkn)->next;
    ...
    if (isSym(*tkn, S_AS))
    {
        *tkn = (*tkn)->next;
        ...
        x = E_VarEntry(sle->u.forLoop.sVar, Tr_allocVar(g_sleStack->lv, S_name(sle->u.forLoop.sVar), varTy), varTy, FALSE);
        E_declare (lenv, x);
    }
    else
    {
    ...
    sle->u.forLoop.var = x->u.var.access;
...
static bool stmtIfBegin(S_tkn *tkn, E_enventry e, Tr_exp *exp)
{
    ...
    *tkn = (*tkn)->next; // consume "IF"
    ...
    E_env      lenv    = E_Env(g_sleStack->env);
    FE_SLE     sle     = slePush(FE_sleIf, pos, g_sleStack->lv, lenv, g_sleStack->exitlbl, g_sleStack->contlbl, g_sleStack->ret_ve);
...
static bool stmtSelect(S_tkn *tkn, E_enventry e, Tr_exp *exp)
{
    ...
    *tkn = (*tkn)->next;
    ...
    FE_SLE sle = slePush(FE_sleSelect, pos, g_sleStack->lv, g_sleStack->env, g_sleStack->exitlbl, g_sleStack->contlbl, g_sleStack->ret_ve);
...
static void stmtProcEnd_(void)
{
    ...
    Tr_procEntryExit(sle->lv,
                     Tr_seqExp(sle->expList),
                     Tr_formals(sle->lv),
                     sle->ret_ve ? sle->ret_ve->u.var.access : NULL,
                     sle->exitlbl,
                     /*is_main=*/FALSE);
}
...
static bool transAssignArgExp(S_tkn *tkn, Tr_expList assignedArgs, Ty_formal *formal)
{
    ...
    if (((*formal)->ty->kind == Ty_procPtr) && ((*tkn)->kind == S_IDENT) && ((*tkn)->next->kind != S_LPAREN))
    {
        ...
        E_enventryList lx = NULL;
        S_symbol name = (*tkn)->u.sym;
        if (proc->returnTy->kind == Ty_void)
            lx = E_resolveSub(g_sleStack->env, name);
        else
            lx = E_resolveFunc(g_sleStack->env, name);
...
// subPtrCall ::= ident ["("] actualArgs [")"]
static bool transSubPtrCall(S_tkn *tkn, E_enventry e)
{
    Ty_proc    proc   = e->u.var.ty->u.procPtr;
    Ty_formal  formal = proc->formals;
    ...
    *tkn = (*tkn)->next;
    ...
    Tr_exp subPtr = Tr_Var(e->u.var.access);
...
static bool stmtCall(S_tkn *tkn, E_enventry e, Tr_exp *exp)
{
    ...
    *tkn = (*tkn)->next; // skip "CALL"
    ...
    E_enventry x = E_resolveVar(g_sleStack->env, name);
    if (x && (x->u.var.ty->kind == Ty_procPtr))
        return transSubPtrCall (tkn, x);
...
static bool procHeader(S_tkn *tkn, S_pos pos, bool isPrivate, bool isFunction, bool forward, Ty_proc *proc)
{
    ...
    *proc = Ty_Proc(name, extra_syms, Temp_namedlabel(label), isPrivate, paramList->first, isStatic, returnTy, forward, /*offset=*/ 0, /*libBase=*/ NULL);

    return TRUE;
}

static bool transProc(S_pos pos, Ty_proc proc, E_enventry *x)
{
...
static bool stmtProcBegin(S_tkn *tkn, E_enventry e, Tr_exp *exp)
{
    ...
    for (Ty_formal formals = proc->formals;
        ...
        E_declare(lenv, E_VarEntry(formals->name, Tr_accessListHead(acl), formals->ty, FALSE));
    ...
    if (proc->returnTy->kind != Ty_void)
    {
        ...
        E_declare(lenv, ret_ve);
    }

    Temp_label exitlbl = Temp_newlabel();

    slePush(isFunction ? FE_sleFunction : FE_sleSub, pos, funlv, lenv, exitlbl, /*contlbl=*/NULL, ret_ve);
    ...
    *tkn = (*tkn)->next; // consume "SUB" | "FUNCTION"
    ...
    if (isSym(*tkn, S_LIB))
    {
        *tkn = (*tkn)->next;
        ...
        E_enventry x = E_resolveVar(g_sleStack->env, sLibBase);
        if (!x)
...
{
    S_pos    pos = (*tkn)->pos;
    FE_SLE   sle = slePush(FE_sleType, pos, g_sleStack->lv, g_sleStack->env, g_sleStack->exitlbl, g_sleStack->contlbl, g_sleStack->ret_ve);
        ...
        *tkn = (*tkn)->next;
        ...
        for (FE_udtEntry f = sle->u.typeDecl.eFirst; f; f=f->next)
        {
            switch(f->kind)
            {
                ...
                {
                    Ty_RecordAddMethod(ty, f->u.methodr);
                    break;
                }
            }
        }
        ...
        E_enventry x = E_TypeEntry(sle->u.typeDecl.sType, ty);
        E_declare (g_sleStack->env, x);
...
static bool stmtWhileBegin(S_tkn *tkn, E_enventry e, Tr_exp *exp)
{
    ...
    FE_SLE     sle      = slePush(FE_sleWhile, pos, g_sleStack->lv, lenv, loopexit, loopcont, g_sleStack->ret_ve);
...
static bool stmtDo(S_tkn *tkn, E_enventry e, Tr_exp *exp)
{
    ...
    Temp_label loopexit = Temp_newlabel();
    Temp_label loopcont = Temp_newlabel();
    FE_SLE     sle      = slePush(FE_sleDo, pos, g_sleStack->lv, lenv, loopexit, loopcont, g_sleStack->ret_ve);
...
static bool stmtReturn(S_tkn *tkn, E_enventry e, Tr_exp *exp)
{
    ...
    *tkn = (*tkn)->next; // consume "RETURN"
    ...
    if (sle->ret_ve)
    {
        ...
        Tr_exp var = Tr_Var(sle->ret_ve->u.var.access);
        Ty_ty ty = Tr_ty(var);
        // if var is a varPtr, time to deref it
        if (ty->kind == Ty_varPtr)
        {
...
static bool funVarPtr(S_tkn *tkn, E_enventry e, Tr_exp *exp)
{
    ...
    *tkn = (*tkn)->next;
    ...
    Tr_exp v;
    if (!varDesignator(tkn, &v, /*doDeRef=*/FALSE))
        return FALSE;
    Ty_ty ty = Tr_ty(v);
    if (ty->kind != Ty_varPtr)
        return EM_error(pos, "This object cannot be referenced.");
...
static bool funStrDollar(S_tkn *tkn, E_enventry e, Tr_exp *exp)
{
    ...
    *tkn = (*tkn)->next;
    ...
    if (fsym)
    {
        E_enventryList lx = E_resolveFunc(g_sleStack->env, fsym);
        if (!lx)
            return EM_error(pos, "builtin %s not found.", S_name(fsym));
        E_enventry func = lx->first->e;
...
{
    Ty_proc proc = Ty_Proc(sym, /*extraSyms=*/NULL, /*label=*/NULL, /*isPrivate=*/TRUE, /*formals=*/NULL, /*isStatic=*/FALSE, /*returnTy=*/retTy, /*forward=*/TRUE, /*offset=*/0, /*libBase=*/0);
    E_enventry e = E_ProcEntry (sym, /*level=*/NULL, proc, parsef);
...
static bool statementOrAssignment(S_tkn *tkn)
{
    if ((*tkn)->kind == S_IDENT)
    {
        if ((*tkn)->next->kind == S_EQUALS)
        {
            E_enventry x = E_resolveVar(g_sleStack->env, (*tkn)->u.sym);
            if (x)
            {
...
F_fragList FE_sourceProgram(FILE *inf, const char *filename, bool is_main, string module_name)
{
    ...
    g_sleStack = NULL;
    slePush(FE_sleTop, /*pos=*/0, lv, env, /*exitlbl=*/NULL, /*contlbl=*/NULL, /*ret_ve=*/NULL);
